...
\begin{itemize}
    ...
    \item \textbf{Absence de mécanisme de retransmission} : Contrairement au Mode 3, aucun timer ne redéchire les données. La fiabilité dépend entièrement de la qualité du lien réseau.
\end{itemize}
...
\begin{itemize}
    \item Données : 10 paquets $\times$ 10 octets = 800 bits
    \item Temps : 0.000000 à 0.002840 s = 0.002840 s
    \item \textbf{Débit} : $\frac{800}{0.002840} \approx 282$ kbps
\end{itemize}

\textit{2. Délai Moyen :}
\begin{itemize}
    \item Moyen $\approx 0.000284$ s (très rapide en l'absence de perte)
\end{itemize}
...
\paragraph{Débit effectif :} 282 kbps (sans perte) | Bloqué indéfiniment (avec perte ACK)
...
\textit{2. Délai Moyen :}
\begin{itemize}
    \item Moyen : $\approx 0.050$ s (dominé par le délai de SN=6 : $0.501972 - 0.001021 = 0.501$ s)
\end{itemize}
...
\begin{itemize}
    \item Données : 10 paquets $\times$ 10 octets = 800 bits
    \item Temps : 0.000000 à 0.001724 s = 0.001724 s
    \item \textbf{Débit} : $\frac{800}{0.001724} \approx 464$ kbps (2.8x plus rapide que Mode 3)
\end{itemize}

\textit{2. Délai Moyen :}
\begin{itemize}
    \item Moyen : $\approx 0.0012$ s (pipelining réduit les délais)
\end{itemize}
